    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon @click="editSaleable(props.item)" v-if="props.item.saleable">下架</v-btn>
  ...
  export default {
    ...
    methods: {
      ...
      async editGoods(oldGoods) {
        ...
        deleteGoods(item){
            this.$http.delete("/item/goods/delete/"+item.id)
                .then(() => {
              // 成功，关闭窗口
              this.$emit("close");
              // 提示成功
            ...
            console.log("调用初始化方法之前")

            console.log("调用初始化方法之后")

        },
        editSaleable(item){
            this.$http.get("/item/goods/editSaleable?saleable="+!item.saleable+"&id="+item.id).then(()=>{
                // 成功，关闭窗口
                this.$emit("close");
                // 提示成功
